...
class utils:
    ...
    @staticmethod
    def simple_index(outer_array):
        """ assume arr of uneven arrays """
        final_inds = []
        final_nums = []
        for i, array in enumerate(outer_array):
            final_inds.extend([i]*len(array))
            final_nums.extend(array)
        return np.array(final_inds), np.array(final_nums)
